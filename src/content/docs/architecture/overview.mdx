---
title: "4.1 Architectural Overview"
---
...
flowchart TB
    subgraph Coordination["Coordination Layer (affine-cortex)"]
        ...
        EX["Executor<br>(workers × environments)"]
    ...
    subgraph Inference["Inference Layer (Chutes + Targon)"]
        GPU["GPU Endpoints<br>(vLLM, OpenAI-compatible)"]
    end
...
3. **The coordination layer** (affine-cortex) runs the scoring pipeline, task pool management, and executor processes described in [Section 3](/mechanism/system-participants/). It orchestrates evaluation by:
