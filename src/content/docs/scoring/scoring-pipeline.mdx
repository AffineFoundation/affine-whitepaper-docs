---
title: "6.1 The Scoring Pipeline"
---
...
```mermaid
flowchart LR
    A["Stage 1<br>Data Collection"] --> B["Stage 2<br>Pareto Filtering"]
    B --> C["Stage 3<br>ELO Rating"]
    C --> D["Stage 4<br>Weight Normalization"]

    A --- A1["Per-miner, per-env<br>avg scores<br>(≥90% completeness)"]
    B --- B1["Remove statistically<br>dominated miners<br>(anti-plagiarism)"]
    C --- C1["Geometric mean → ranking<br>→ ELO update<br>→ rank-based weights"]
    D --- D1["Accumulate subsets<br>→ 1% floor<br>→ normalize to 1.0"]
